...
void TitleCard_Draw(PlayState* play, TitleCardContext* titleCtx) {
    ...
    if (titleCtx->alpha != 0) {
        ...
        doubleWidth = width * 2;
        titleX1 = WIDE_INCR((titleCtx->x * 4) - (width * 2), (WIDE_GET_RATIO * 100.0f));
...
void Actor_DrawLensOverlay(GraphicsContext* gfxCtx) {
    ...
    gDPSetTileSize(POLY_XLU_DISP++, G_TX_RENDERTILE,
                   WIDE_DIV(((SCREEN_WIDTH / 2 - LENS_MASK_WIDTH) << 2), WIDE_GET_RATIO),
                   (SCREEN_HEIGHT / 2 - LENS_MASK_HEIGHT) << 2,
                   (WIDE_DIV(SCREEN_WIDTH / 2 + LENS_MASK_WIDTH - 1, WIDE_GET_RATIO) + 37) << 2,
