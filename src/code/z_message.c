...
#include "assets/textures/hud/hud.h"
#endif
...
void Message_SetTitleCardInfo(struct PlayState* play, TitleCardInfo* info) {
    if (info == NULL) {
        PRINTF("[HackerOoT:Error]: TitleCardInfo is NULL!\n");
    }
...
void Message_DrawSceneTitleCard(PlayState* play, Gfx** gfxP) {
    ...
    if (msgCtx->titleCardInfo != NULL) {
        ...
        gDPLoadTextureBlock(gfx++, gSceneTitleCardGradientTex, G_IM_FMT_I, G_IM_SIZ_8b, 64, 1, 0,
                            G_TX_NOMIRROR | G_TX_WRAP, G_TX_NOMIRROR | G_TX_WRAP, 6, G_TX_NOMASK, G_TX_NOLOD,
                            G_TX_NOLOD);
        gSPTextureRectangle(gfx++, 0, msgCtx->titleCardInfo->gradientWidth << 2, 320 << 2,
                            (msgCtx->titleCardInfo->gradientWidth + msgCtx->titleCardInfo->gradientHeight) << 2,
...
void Message_DrawTextChar(PlayState* play, void* textureImage, Gfx** p) {
    ...
    gSPTextureRectangle(gfx++, x << 2, y << 2, (x + WIDE_MULT(sCharTexSize, WIDE_GET_RATIO)) << 2,
                        (y + sCharTexSize) << 2, G_TX_RENDERTILE, 0, 0, WIDE_DIV(sCharTexScale, WIDE_GET_RATIO),
                        sCharTexScale);
    *p = gfx;
}
...
void Message_DrawTextboxIcon(PlayState* play, Gfx** p, s16 x, s16 y) {
    ...
    gSPTextureRectangle(gfx++, x << 2, y << 2, WIDE_INCR(((x + sCharTexSize) << 2), -(u16)(WIDE_GET_4_3 * 10.0f)),
                        (y + sCharTexSize) << 2, G_TX_RENDERTILE, 0, 0, WIDE_DIV(sCharTexScale, WIDE_GET_RATIO),
                        sCharTexScale);
...
u16 Message_DrawItemIcon(PlayState* play, u16 itemId, Gfx** p, u16 i) {
    ...
    gSPTextureRectangle(gfx++, (WIDE_INCR(msgCtx->textPosX, 3) + R_TEXTBOX_ICON_XPOS) << 2, R_TEXTBOX_ICON_YPOS << 2,
                        WIDE_INCR((msgCtx->textPosX + R_TEXTBOX_ICON_XPOS + R_TEXTBOX_ICON_DIMENSION),
                                  (itemId < ITEM_MEDALLION_FOREST ? -5 : -3))
                            << 2,
                        (R_TEXTBOX_ICON_YPOS + R_TEXTBOX_ICON_DIMENSION) << 2, G_TX_RENDERTILE, 0, 0,
        ...
        switch (character) {
            ...
            case MESSAGE_BOX_BREAK:
                ...
                *gfxP = gfx;
                return;
            case MESSAGE_SHIFT:
                msgCtx->textPosX += WIDE_INCR(MSG_BUF_DECODED[++i], (msgCtx->textId == 0x009E ? -16 : -8));
                break;
            case MESSAGE_TEXTID:
                msgCtx->textboxEndType = TEXTBOX_ENDTYPE_HAS_NEXT;
                if (msgCtx->msgMode == MSGMODE_TEXT_DISPLAYING) {
...
void Message_DrawTextBox(PlayState* play, Gfx** p) {
    ...
    if (msgCtx->textBoxType == TEXTBOX_TYPE_OCARINA) {
        ...
        gDPLoadTextureBlock_4b(gfx++, gOcarinaTrebleClefTex, G_IM_FMT_I, 16, 32, 0, G_TX_MIRROR, G_TX_MIRROR,
                               G_TX_NOMASK, G_TX_NOMASK, G_TX_NOLOD, G_TX_NOLOD);
        gSPTextureRectangle(gfx++, WIDE_INCR(R_TEXTBOX_CLEF_XPOS, 20) << 2, R_TEXTBOX_CLEF_YPOS << 2,
                            WIDE_INCR((R_TEXTBOX_CLEF_XPOS + 16), 20) << 2, (R_TEXTBOX_CLEF_YPOS + 32) << 2,
    ...
    if (msgCtx->msgLength != 0) {
        ...
        if (msgCtx->msgMode >= MSGMODE_OCARINA_PLAYING && msgCtx->msgMode < MSGMODE_TEXT_AWAIT_NEXT &&
            ...
            if (msgCtx->msgMode == MSGMODE_SONG_PLAYBACK) {
                ...
                for (notePosX = WIDE_INCR(R_OCARINA_BUTTONS_XPOS, 16), i = 0; i < r; i++) {
                    gDPPipeSync(gfx++);
                    ...
                    gSPTextureRectangle(gfx++, notePosX << 2,
                                        ...
                                        G_TX_RENDERTILE, 0, 0, 1 << 10, 1 << 10);
                    notePosX += WIDE_INCR(R_OCARINA_BUTTONS_XPOS_OFFSET, -4);
                }
            }

            if (msgCtx->msgMode != MSGMODE_SCARECROW_LONG_RECORDING_START &&
                msgCtx->msgMode != MSGMODE_MEMORY_GAME_START) {
                for (notePosX = WIDE_INCR(R_OCARINA_BUTTONS_XPOS, 16), i = 0; i < 8; i++) {
